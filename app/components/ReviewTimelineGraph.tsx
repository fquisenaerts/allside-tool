...
export function ReviewTimelineGraph({ data, explanation }: ReviewTimelineGraphProps) {
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center justify-center h-[300px] text-gray-500">
            <p>
              No timeline data available. Once reviews with dates are collected, this chart will show the distribution
  ...
  const generatePersonalizedExplanation = () => {
    ...
    let contextExplanation = `This timeline shows ${totalReviews} reviews over a ${dateRange}-day period. `

    if (trend === "increasing") {
      contextExplanation += `There's a positive trend with review volume increasing over time, which suggests growing customer engagement. `
    } else if (trend === "decreasing") {
      contextExplanation += `There's a downward trend in review volume, which might indicate decreasing customer engagement. `
    } else {
      contextExplanation += `The review volume has remained relatively stable over this period. `
    }

    contextExplanation += `The highest number of reviews (${peakData.count}) was received on ${peakDate}. `

    if (peakData.count > 3 * (totalReviews / data.length)) {
      contextExplanation += `This spike might correspond to a specific event, promotion, or product launch that generated increased customer feedback.`
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-gray-600">{generatePersonalizedExplanation()}</p>
    </div>
  )
}

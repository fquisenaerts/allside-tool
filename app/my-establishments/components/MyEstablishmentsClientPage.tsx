...
export default function MyEstablishmentsClientPage() {
  ...
  const getEstablishmentSourceType = (establishment: any): string => {
    ...
    if (url.includes("google.com/maps") || type.includes("google")) {
      ...
      return "trustpilot"
    } else if (url.includes("airbnb") || type.includes("airbnb")) {
      return "airbnb"
  ...
  const handleViewAnalysis = (id: string) => {
    router.push(getLocalizedUrl(`/establishment/${id}`, language))
  }
  ...
  const performMasterAnalysis = () => {
    ...
    router.push(`${getLocalizedUrl("/analyze", language)}?bulk=true&type=${encodedType}&urls=${encodedUrls}`)
  }
  ...
  const getSourceTypeBadge = (establishment: any) => {
    ...
    switch (sourceType) {
      ...
      case "airbnb":
        return <Badge className="bg-red-500">{t("myEstablishments.sourceTypes.airbnb")}</Badge>
      case "xls":
  ...
  const filterOptions = useMemo(
    () => [
      ...
      { label: t("myEstablishments.filters.airbnb"), value: "airbnb" },
        ...
        <div className="container mx-auto px-4 py-12">
          ...
          {loading ? (
            ...
            <div className="bg-white p-8 rounded-lg text-center">
              ...
              <Button
                onClick={() => router.push(getLocalizedUrl("/analyze", language))}
